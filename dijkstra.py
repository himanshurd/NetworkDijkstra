...
def path(parent, src_ip, dest_ip):
    make_path = []
    
    while src_ip != dest_ip:
        make_path.append(dest_ip)
        dest_ip = parent[dest_ip]
    if make_path:
        make_path.append(dest_ip)
        make_path.reverse()
...
def check_dist_and_neighbors(curr,distance,routers,parent,to_visit):
         for neighbor in routers[curr]["connections"]:
            if neighbor in to_visit:    
                calculate_dist = distance[curr]  + routers[curr]["connections"][neighbor]["ad"]
                if calculate_dist < distance[neighbor]:
                    distance[neighbor] = calculate_dist
...
def dijkstras_shortest_path(routers, src_ip, dest_ip):
    ...
    **Strong recommendation**: make functions to do subtasks within this
    ...
    while to_visit:
        curr = next(iter(to_visit))
        for node in to_visit:
            if distance[node] < distance[curr]:
                curr = node
        to_visit.remove(curr)
    
        check_dist_and_neighbors(curr,distance,routers,parent,to_visit)

    dest_ip = find_router_for_ip(routers, dest_ip)                
    routing_path = path(parent,src_ip,dest_ip)
    return routing_path
#------------------------------
